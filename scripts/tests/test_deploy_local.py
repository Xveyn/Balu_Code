...
def _run(env, install_root, *extra):
    return subprocess.run(
        ["bash", str(SCRIPT), "--install-root", str(install_root), *extra],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
...
def test_successful_deploy_installs_and_leaves_the_service_running(
    env_and_state, install_root, artefact
):
    env, state, _ = env_and_state

    result = _run(env, install_root, "--artefact", str(artefact))
    ...
    assert list(install_root.glob("balu_code.bak-*")), "previous install should be kept"


def test_a_plugin_that_never_mounts_is_rolled_back(env_and_state, install_root, artefact):
...
def test_unknown_owner_fails_before_the_service_is_stopped(env_and_state, install_root):
    ...
    result = _run(env, install_root, "--owner", "nosuchuser:nosuchgroup")
...
def test_owner_is_derived_from_the_systemd_unit(env_and_state, install_root, artefact):
